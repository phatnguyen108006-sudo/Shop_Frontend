...
export default function OrderTrackingPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      if (user?.role === "admin") {
        ...
        else setError("Không tìm thấy lịch sử đơn hàng của số điện thoại này.");
      } else {
  ...
  const renderHeader = () => {
    ...
    if (user) return "Đơn hàng của bạn";
      ...
      <div className="container mx-auto px-4 max-w-2xl">
        <div className="text-center mb-10 animate-in slide-in-from-bottom-4 duration-500">
          <div className="inline-flex items-center justify-center p-3 bg-white rounded-2xl shadow-sm mb-4">
            {user?.role === "admin" ? <UserCog className="text-blue-600" size={32} /> : user ? <User className="text-green-600" size={32} /> : <Truck className="text-black" size={32} />}
          </div>
          <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight mb-3">{renderHeader()}</h1>
          <p className="text-gray-500 max-w-md mx-auto">{renderDescription()}</p>
        </div>
          ...
          <div className={`bg-white p-8 rounded-2xl shadow-lg border transition-all duration-300 mb-10 ${user?.role === "admin" ? "border-blue-100 shadow-blue-50" : "border-gray-100"}`}>
            ...
            <div className="space-y-5">
              ...
              <button
                ...
                `}
              >
                {loading ? <div className="w-6 h-6 border-3 border-white/30 border-t-white rounded-full animate-spin" /> : <>Tra cứu ngay <ArrowRight size={20} /></>}
              </button>
            </div>
          </div>
        )}
            ...
            >
              <div className="bg-gray-50/80 px-6 py-4 flex flex-wrap justify-between items-center gap-4 border-b border-gray-100">
                <div>
                  ...
                  <p className="font-mono font-bold text-lg text-gray-900 flex items-center gap-2">#{String(order.id || order._id).slice(-6).toUpperCase()}</p>
                </div>
